...
local function InitializeInventory(inventoryId, data)
    return EnsureInventoryLoaded(inventoryId, data)
end
...
function SetInventory(identifier, items, reason)
    ...
    if player then
        ...
        if not player.Offline and InventoryShouldLog('set', identifier) then
    ...
    elseif Inventories[identifier] then
        ...
        MarkInventoryDirty(identifier)
    end

    if InventoryShouldLog('set', identifier) then
...
function GetInventory(identifier)
    ...
    return EnsureInventoryLoaded(identifier)
end
...
-- ============================================================================
-- keep-companion (and some other resources) expect qb-inventory to expose
-- getGloveboxes(plate) and getTruck(plate) exports which return an object with
-- an `items` field.
--
-- This qb-inventory stores vehicle inventories in the `inventories` table using
-- identifiers in the format:
--   glovebox-<plate>
--   trunk-<plate>
--
-- These exports are read-only and are safe to call even if the inventory hasn't
-- been opened yet (lazy loads from DB when possible).

local function GetOrLoadInventoryItems(identifier)
    if not identifier or type(identifier) ~= 'string' then return {} end

    if Inventories and Inventories[identifier] and Inventories[identifier].items then
        return Inventories[identifier].items
    end
...
exports('getGloveboxes', function(plate)
    local identifier = 'glovebox-' .. tostring(plate or '')
    return { items = GetOrLoadInventoryItems(identifier) }
end)

exports('getTruck', function(plate)
    local identifier = 'trunk-' .. tostring(plate or '')
    return { items = GetOrLoadInventoryItems(identifier) }
end)
...
function AddItem(identifier, item, amount, slot, info, reason)
    ...
    if InventoryShouldLog('item', identifier) then
        local invName = player and GetPlayerName(identifier) .. ' (' .. identifier .. ')' or identifier
