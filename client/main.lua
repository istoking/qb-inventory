...
local function IsInventoryInputBlocked()
    -- Prevent hotbar / slot keybinds (1-5) from firing while other UIs/minigames
    -- intentionally keep game input (e.g. bl_ui skillchecks).
    if IsPauseMenuActive() then return true end

    -- Most UIs (including bl_ui) will set NUI focus; if so, don't let hotbar keys fire.
    if IsNuiFocused() then return true end

    -- Additional explicit block from bl_ui (or any other resource setting this flag)
    if LocalPlayer and LocalPlayer.state and LocalPlayer.state.bl_ui_active then return true end

    -- Fallback if the statebag isn't available for some reason.
    if GetResourceState('bl_ui') == 'started' then
        local ok, playing = pcall(function()
            return exports['bl_ui']:IsPlaying()
        end)
...
RegisterNetEvent('qb-inventory:client:openInventory', function(items, other)
    ...
    local first = charinfo and charinfo.firstname or ''
...
RegisterCommand('toggleHotbar', function()
    if IsInventoryInputBlocked() then return end
